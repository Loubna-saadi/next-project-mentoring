...
const MentorsCards = () => {
  ...
  const handleSearch = () => {
    ...
    if (query.trim() === '') {
      console.log('Query is empty, resetting filtered mentors');
      setFilteredMentors(mentors);
    } else {
      const filtered = mentors?.filter((mentor) => {
        if (Array.isArray(mentor.category)) {
          const matches = mentor.category.some((cat) =>
            cat.toLowerCase().includes(query.toLowerCase())
          );
          console.log(`Mentor: ${mentor.username}, Categories: ${mentor.category}, Matches: ${matches}`);
          return matches;
        }
        return false;
      }) || null;
    ...
    <div>
      <div className='mb-10 mt-10 items-center flex flex-col gap-4'>
        ...
        <div className="flex w-full max-w-sm items-center space-x-2">
          <Input
            type="text" // Change to 'text' to allow for all category input
        ...
        <div className="px-4 mx-auto sm:px-6 lg:px-8 max-w-7xl">
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            {(filteredMentors || []).map((mentor) => (
              <div key={mentor._id} className="overflow-hidden bg-white rounded shadow w-full h-full max-w-xs">
                <div className="p-4">

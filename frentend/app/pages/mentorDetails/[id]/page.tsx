...
import StripeProvider from '@/app/_components/StripeProvider';
...
//DONE
    ...
    useEffect(() => {
        ...
        const fetchMentor = async () => {
          try {
            const response = await axios.get(`http://localhost:5000/api/getMentor/${id}`);
            if (response.data.success) {
              setMentor(response.data.data); // Assuming 'data' contains mentor details
            } else {
              console.error('Error fetching mentor:', response.data.error);
            }
          } catch (error) {
    ...
    const handleSubmit = async (e: any) => {
      ...
      e.preventDefault();
      localStorage.removeItem('bookingProcessed');
      const bookingData = {
        mentor: id,
        quantity: "1",
      ...
      try {
        const response = await fetch("http://localhost:5000/create-checkout-session", {
          ...
          body: JSON.stringify({
            items: [
              { price_data: bookingData.appointmentPrice, quantity: 1,mentor_photo:mentor.photo,name:mentor.username }  // Assuming this is the correct format
            ]
          })
        });
...
<div className='mt-8' >
    <div className='flex items-center gap-4 mb-0'>
        <figure className='max-w-[200px] max-h-[200px]'><img src={mentor.photo} alt="" /></figure>
        <div>
            ...
            {mentor.category}</span>
                <h3 className='text-[22px] leading-9 font-bold text-black mt-3'>{mentor.username}</h3>
        ...
        <div className="grid grid-cols-1 gap-8 mt-10 text-center lg:mt-9 sm:gap-x-8 md:grid-cols-3">
            <div>
                <h3 className="font-bold text-7xl">
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-green-600 to-blue-600"> {mentor.YearsInbusiness} + </span>
                </h3>
            ...
            <div>
                <h3 className="font-bold text-7xl">
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-green-600 to-blue-600">{mentor.NumberOfMentees}</span>
                </h3>
            ...
            <div>
                <h3 className="font-bold text-7xl">
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-green-600 to-blue-600"> {mentor.YearsOfMentoring} +</span>
                </h3>
  ...
  <form onSubmit={handleSubmit}>
      <div>
        ...
        <div className="mt-4 grid max-w-3xl gap-x-4 gap-y-3 sm:grid-cols-2 md:grid-cols-3">
          <div className="relative">
            <input className="peer hidden" id="radio_1" type="radio" name="radio" value="300" onChange={handleServiceChange} />
          ...
          <div className="relative">
            <input className="peer hidden" id="radio_2" type="radio" name="radio" value="600" onChange={handleServiceChange} />
            <span className="absolute right-4 top-1/2 box-content block h-3 w-3 -translate-y-1/2 rounded-full border-8 border-gray-300 bg-white peer-checked:border-emerald-400"></span>
            <label className="flex h-full cursor-pointer flex-col rounded-lg p-4 shadow-lg shadow-slate-100 peer-checked:bg-emerald-600 peer-checked:text-white" htmlFor="radio_2">
              ...
              <span className="text-xs uppercase">with follow-up of results</span>
            </label>
          </div>
          <div className="relative">
            <input className="peer hidden" id="radio_3" type="radio" name="radio" value="800" onChange={handleServiceChange} />
            <span className="absolute right-4 top-1/2 box-content block h-3 w-3 -translate-y-1/2 rounded-full border-8 border-gray-300 bg-white peer-checked:border-emerald-400"></span>
            <label className="flex h-full cursor-pointer flex-col rounded-lg p-4 shadow-lg shadow-slate-100 peer-checked:bg-emerald-600 peer-checked:text-white" htmlFor="radio_3">
              ...
              <span className="text-xs uppercase">meeting in real life</span>
            </label>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="relative mt-4 w-56">
          ...
          <DatePicker
            ...
            classNames={{
              input: () => "datepicker-input block w-full rounded-lg border border-emerald-300 bg-emerald-50 p-2.5 pl-10 text-emerald-800 outline-none ring-opacity-30 placeholder:text-emerald-800 focus:ring focus:ring-emerald-300 sm:text-sm"
            }}
          />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="mt-4 grid grid-cols-4 gap-2 lg:max-w-xl">
          {["12:00", "14:00", "09:00", "15:00"].map((t) => (
            <button type="button" key={t} onClick={() => handleTimeChange(t)} className={`rounded-lg px-4 py-2 font-medium ${time === t ? 'bg-emerald-700 text-white' : 'bg-emerald-100 text-emerald-900'} active:scale-95`}>
              {t}
            </button>
          ))}
        </div>
      ...
      {/* <button onClick={checkout}  className="mt-8 w-56 rounded-full border-8 border-emerald-500 bg-emerald-600 px-10 py-4 text-lg font-bold text-white transition hover:translate-y-1">Pay Now</button> */}
      {/* <StripeProvider>
        <CheckoutForm bookingData={{ service, date, time, client:Client?.userId  }} handleBookingSuccess={handleBookingSuccess} />
      </StripeProvider> */}
